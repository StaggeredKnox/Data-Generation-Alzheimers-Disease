...
args.work_dir = "/home/rajk/Desktop/IE_synthetic_data_work/Explaining_Shape_Variability-master/src/DeepLearning/compute_canada/guided_vae"
...
#print(torch.isnan(up_transform_list[0].detach()).any())
...
def objective(trial):
    ...
    args.lr = trial.suggest_float("learning_rate", 0.00001, 0.0001, log=True)
    ...
    args.clip_value = trial.suggest_float("clip_value", 5.0, 15.0, step=1.0)
    ...
    info = run(model, train_loader, val_loader, args.epochs, optimizer, scheduler, writer, device, args.beta, args.wcls, args.guided, args.clip_value)
    #info = run(model, train_loader, val_loader, 5, optimizer, scheduler, writer, device, args.beta, args.wcls, args.guided)
    #exit()#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if sap_score > 0.20:
        ...
        key = f"{trial.number}"
        value = {"sap_score": float(sap_score), "pcc": float(pcc)} | info | vars(args)
        log_model_info[key] = value

        # saving the model info

        dict_path = "/home/rajk/Desktop/IE_synthetic_data_work/Explaining_Shape_Variability-master/src/DeepLearning/compute_canada/guided_vae/log_model_info.json"

        with open(dict_path, 'w') as json_file:
            json.dump(log_model_info, json_file)

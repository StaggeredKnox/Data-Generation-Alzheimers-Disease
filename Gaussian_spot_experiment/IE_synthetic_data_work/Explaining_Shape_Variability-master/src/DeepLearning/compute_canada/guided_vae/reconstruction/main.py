...
def objective(trial):
    ...
    info = run(model, train_loader, val_loader, args.epochs, optimizer, scheduler, writer, device, args.beta, args.wcls, args.guided)
    ...
    if sap_score > 0.20:
        ...
        key = f"{trial.number}"
        value = {"sap_score": float(sap_score), "pcc": float(pcc)} | info
